---
title: "Install SE2 in your cloud environment"
---
...
1. Deploy a Kubernetes cluster into your cloud provider of choice (if you have a pre-existing one, that works too!).
    ...
    - If you would like assistance with storage, send us an [email](team@suborbital.dev)!
...
## Set up DNS
When you deploy, a `LoadBalancer` is created for the builder called `se2-builder-service`. You should wait until an external IP address is provisioned for the service (use the `get svc` command above to check), and then create a DNS `A` record that matches the domain name you entered during setup. This will allow the builder to automatically provision a TLS certificate and connect with the [plugin editor](/how-to/plugins/editor) securely.
...
```
_As always, don't hesitate to reach out if you have any issues with this or any other aspect of SE2!_
